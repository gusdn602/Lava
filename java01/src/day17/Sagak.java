...
public class Sagak implements Comparable{
	...
	public int compareTo(Object o) {
		//이 함수는 Compartable 인터페이스가 가지고 있는 추상함수 이므로
		//이 인터페이스를 구현해서 클래스를 제작하는 경우에는 
		//반드시 이 함수 오버라이드를 해줘야 한다.
		//이때 반환값이 음수인 경우는 정렬시 입력된 데이터와 자리를 바꾸지 않는다는 이야기이다..
		// ==> 매개변수로 입력되는 데이터가 더 크다.
		// 반환값이 0인 경우는 같은경우이다.
		//반환값이 양수인 경우는 정렬시 입력된 데이터와 자리를 바꾼다.
		
		// Object 타입으로 입력된 데이터를 Sagak타입으로 강제형변환을 해줘야 한다.
			Sagak s = (Sagak) o;
