...
/*
	참고]
		거의 대부분의 언어는 문자열 자체를 변경하는 기능은 없다
...
*/
		...
		//금요일 우리는 아주편한 기능을 하나 배웠는데 그것이 StringTokenizer 였다.
		StringTokenizer token = new StringTokenizer(tmp);
		
		//이제 공백을 기준으로 잘려진 결과가 버퍼 메모리에 저장이 되어있으므로 하나씩 꺼내서 담으면 되겠다.
		// 참고로 ArrayList는 사이즈를 지정해주지 않아도 만들수 있으므로 그대로 담자.
		
		//ArrayList 만들고 
		ArrayList list = new ArrayList();
		
		//token에서 하나씩 꺼내서 채워 넣자.
		while(token.hasMoreElements()) { //아직 버퍼 메모리에 꺼낼 데이터가 남아있냐?
