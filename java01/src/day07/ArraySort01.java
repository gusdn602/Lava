...
public class ArraySort01 {
	...
	public static void main(String[] args) {
		...
		for(int i= 0; i<lotto.length - 1; i++) {
			//여기서 비교할 데이터는 맨 마지막 데이터는 비교할 필요가 없으므로 맨 마지막 바로 이전까지만 뽑아서 비교해주면 된다.
			
			//꺼낸 데이터 이후부터 맨 마지막까지 하나씩 꺼내서 비교한다.
			for(int j = i+1; j <lotto.length; j++) {
				if(lotto[i] > lotto[j]) {
					//이 겨우는 뒤에서 뽑아온 데이터가 더 작은 경우이므로 
					//현재 자리와 데이터를 교체해야 한다.
					// 이때 현재 자리에 데이터를 교체하는 순간 이전 데이터의 기억은 사라지므로
					//변수에 주소를 기억시켜 놓는다.
					int tmp = lotto[i];
					lotto[i] = lotto[j];
					lotto[j] = tmp;
		...
		// 7. 출력하고
		JOptionPane.showMessageDialog(null, "### 당첨예정번호 ###\n" + Arrays.toString(lotto));
